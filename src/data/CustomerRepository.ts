...
export const CustomerRepository = {
  async getCustomers(query?: string | null): Promise<Profile[]> {
    let q = supabase.from('profiles').select('id, full_name, email, phone, address, role, created_at').eq('role', 'customer');
  ...
  async getCustomer(id: string): Promise<Profile | null> {
    const { data, error } = await supabase.from('profiles').select('id, full_name, email, phone, address, role, created_at').eq('id', id).maybeSingle();
    if (error) throw error;
